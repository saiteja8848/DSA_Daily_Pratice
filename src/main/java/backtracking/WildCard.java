...
public class WildCard {
	...
	private boolean is_match(int m, int n, String p, String s) {
		...
		if (n < 0 && m >= 0) {
			/*
			 * if str = "" && pattern = "***" then return true; because we can neglect the *
			 * also else str = "" && pattern = "sd**" return false; because after neglecting
			 * the * pattern has some more character
			 * 
			 */
			for (int i = 0; i <= m; i++) {
				if (p.charAt(i) != '*')
